...
%% Макрос для введения. Совместим со старым стилевиком.
    ...
    Заметим что
    \begin{gather*}
        P(\Delta Y = i|Y_t=I) = \sum_{j=1}^{I - i} A(i + j)Q(i)
    \end{gather*}
    Тогда значение \EYdelta можно расписать в виде:
    $\EYdelta = \sum_{i=1}^I i \cdot \sum_{j=1}^{I - i} A(i + j)Q(i) = \sum_{i=1}^I Q(i) \sum_{j=i}^{I} j \cdot A(j - i)$
    \begin{align*}{5}
        Q(0)A(1) + && 2*Q(0)A(2) && + 3*Q(0)A(3) && + {}\ldots{} && + I*Q(0)A(I) + & \\
        && Q(1)A(2)   && + 2*Q(1)A(3) && + {}\ldots{} && + (I-1)*Q(1)A(I)  + &\\
    ...
    Это будет нашей нижней оценкой на \EYdelta
    ...
    $i \geq 2 \n i \geq 1$
    \begin{gather*}
        \frac{(j + 1)*Q(i)*A(i + j + 1)}{j* Q(i) \cdot A(i + j)} \\
        ...
        =  \frac{j + 1}{j \cdot (i + j + 1)} \cdot  \frac{c*(n - r) + c*r - i - j - 1}{n - r} \cdot r \leq \\
        \leq \frac{1}{2} \cdot \frac{c*(n - r)}{n - r} \cdot r = \frac{c \cdot r}{2}
    \end{gather*}
    ...
    Введем значение $\tau \equiv \exp(-r + r*c) \cdot c \cdot r$ \\
    \begin{gather*}
        \tau^* = q^{n - I} \cdot c \cdot r = (1 - r/n)^n \cdot (1 - c) \cdot c \cdot r < \exp(-r + r*c) \cdot c \cdot r = \tau\\
        \tau^* = (1 - r/n)^{n - r}*(1 - c) \cdot c \cdot r \cdot (1 - r/n)^{r}*(1 - c) \\
        > \exp(-r + r*c) \cdot c \cdot r \cdot (1 - r^2/2n)^{1 - c} > \\
        \exp(-r + r*c) \cdot c \cdot r \cdot (1 - r^2/2n) > \exp(-r + r*c) \cdot c \cdot r \cdot (1 - o(1)) = \tau \cdot (1 - o(1))\\
    \end{gather*}
    Заметим так же:
    \begin{gather*}
        q^{I + 3} = (1 - r/n)^{c + 3} <~\eqref{exp:less}< \exp(-r \cdot c) \cdot (1 - r/n)^3 < \exp(-r*c)\\
    \end{gather*}
    Обозначим $k \equiv \exp(-r*c) \cdot c \cdot r^2$ \\
    Тогда сумма всей таблицы оценивается сверху как: \\
    \begin{gather*}
        \tau^* \cdot (1 + q^{I + 3} \cdot c \cdot r^2)  < \exp(-r + r*c) \cdot c \cdot r \cdot (1 + \exp(-r*c) \cdot c \cdot r^2) < \tau \cdot (1 + k)
    \end{gather*}

    Из чего сделаем вывод что $\EYdelta  \in (\tau \cdot (1 - o(1)), \tau \cdot (1 + k))$ \\
    ...
    \[
        \EZdelta = (1 - c)/K
    \]
    Заметим что $\EZdelta$ возрастает с убыванием $c$, когда как \EYdelta убывает.\\
    ...
    Тогда рассмотрим для фиксированного $K$ такие два значения $c_{upper}$ и $c_{lower}$ для которых мы можем быть уверены что:
